...
usage() {
  cat <<'EOF'
usage: ./safe-push.sh [--remote <name>] [--branch <name>] [--test-cmd <cmd>] [--always-test] [--no-test]
...
Default behavior:
  ...
  5. push only if every previous step succeeds
...
if [[ "${behind}" != "0" ]]; then
  printf '[2/4] rebase onto %s/%s (ahead=%s behind=%s)\n' "${REMOTE}" "${BRANCH}" "${ahead}" "${behind}"
...
EOF
  ...
  printf '[2/4] remote is not ahead (ahead=%s behind=%s); skip rebase\n' "${ahead}" "${behind}"
fi

if [[ "${NO_TEST}" == "1" ]]; then
  printf '[3/4] skip tests (--no-test)\n'
elif [[ "${rebase_happened}" == "1" || "${ALWAYS_TEST}" == "1" ]]; then
  printf '[3/4] run tests: %s\n' "${TEST_CMD}"
...
EOF
  ...
  printf '[3/4] skip tests (no rebase happened)\n'
fi

printf '[4/4] push %s HEAD:%s\n' "${REMOTE}" "${BRANCH}"
