...
> Type: `draft`
> Updated: `2026-04-25`
> Summary: 合并 Web Setup 与 Web Admin 的产品改造要求，记录确认版独立页面 Mock，汇总后端实现路径、接口改造方案与旧路径清理结论；截至 2026-04-25，本方案已被 #425 / #424 / #426 作为落地依据执行。
...
1. 本文最初形成于方案调研阶段。
2. 截至 `2026-04-25`，对应子 issue `#425 / #424 / #426` 已按本文方向完成后端前置、页面落地与旧路径清理。
3. 后续若产品形态再发生变化，应继续以更新本文和配套 Mock 的方式同步，而不是恢复旧路径。
...
2. 增加“验证成功后机器人主动通知”的后端动作封装：
   - 若当前机器人已有最近使用过的飞书会话，则统一发文本提示（成功建立连接、基础设置已通过）
...
2. 服务端直接通过飞书 API 发测试提示；投递目标由服务端按当前机器人最近使用过的飞书会话自动解析，不依赖前端“活跃会话选择”。
...
6. 如果当前机器人还没有任何最近使用过的飞书会话，则接口返回“目标不可达”，前端需提示用户先在飞书里给机器人发送一条消息后再重试。
...
4. 网关入站事件命中规则：
   - 仅匹配同 `gateway_id`
   - 事件订阅仅匹配固定验证词
   - 回调仅匹配对应测试按钮 action（沿用现有 `page_action` 载体，不新增新的 callback payload kind）
...
4. 当前实现使用“该机器人最近使用过的飞书 surface”作为投递目标，并优先复用该 surface 的 `chat_id` / `open_id`。
5. 如果当前机器人还没有任何可复用的最近 surface，则返回 `feishu_app_test_target_unavailable`，由前端展示 fallback 提示，不在服务端做隐式猜测。
6. 该目标解析结果仅用于服务端调用飞书 API 发测试提示，不改变用户交互流程。
...
2. 响应体：
   ...
   - `message`（已发出测试提示）
3. 失败场景：
   - 若无最近可投递 surface，返回 `409 feishu_app_test_target_unavailable`
