...
> Type: `draft`
> Updated: `2026-04-06`
> Summary: 保留 Web 安装管理总体方向，并把 setup 向导交互重设计拆到独立文档。
...
它需要覆盖完整链路，但具体交互以 [web-setup-wizard-redesign.md](./web-setup-wizard-redesign.md) 为准：
...
setup 首屏应该是线性向导，不是多 App 工作台：

- setup 只聚焦“先完成一个可工作的接入”
- 多 App 的新增、编辑、启停和观察放到完成 setup 之后的 admin 页面
...
推荐向导步骤如下，页面细节、文案和显隐规则以 [web-setup-wizard-redesign.md](./web-setup-wizard-redesign.md) 为准：
...
2. 创建并连接飞书应用
   - 创建企业自建应用
   - 添加机器人能力
   - 录入 App ID / App Secret
   - 点击 `测试并继续` 后自动验证
3. 权限导入
   - 导出 scopes JSON
   - 打开当前 App 的权限管理页
   - 进入“批量导入/导出权限”，粘贴 JSON 并点击“保存并申请开通”
4. 事件订阅
   - 单独展示事件清单与用途
   - 点击“订阅方式”，默认就是长连接并保存
5. 回调订阅方式
   - 明确告诉用户当前版本使用飞书长连接，不需要填写回调 URL
   - 在“回调配置”里把“回调订阅方式”设为长连接并保存
6. 机器人菜单
   - 单独展示有效菜单 key 与按钮用途
7. 发布验收
   - 用户发版后，由系统执行生效检查
