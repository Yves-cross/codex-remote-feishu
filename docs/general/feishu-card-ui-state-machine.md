...
- turn-owned 的投递策略当前已经改成“高价值文本 reply、过程卡仍顶层 append”：
  ...
  - assistant-facing 的 `agent_message` delta 会在 upstream 标记 `phase=final_answer` 或 `phase=commentary` 时投影成 `assistant_stream` CardKit streaming card：同一 turn 的 item started 先创建 `streaming_mode=true` 的 CardKit 卡片实体并回复到 turn anchor。卡片 body 当前固定包含两个 markdown 元素：`content` 专门承载真实正文，`loading` 专门承载等待态提示。若首段正文尚未到达，首包就直接在 `loading` 元素写入 waiting 第 1 帧 `.`；后续 commentary / final delta 继续只把真实全量正文写入 `content` 元素，保持飞书 CardKit 的原生 streaming 路径，不再把等待态字符拼进正文前缀。卡片配置继续对齐飞书官方默认节奏：`print_frequency_ms=70`、`print_step=1`、`print_strategy=fast`；服务端正文更新仍节流在单卡 `10 次/秒` 预算内。若首段正文尚未到达，或上一段正文已经落地但下一段 partial 还没回来，orchestrator 会在 `400ms` waiting tick 上递增 `LoadingStep`，projector 把它投影成 `loading` 元素的 `.` / `..` / `...` 三帧循环；gateway 在真正写入 CardKit `loading` 元素前，会把这三个点包成一个中性色 `text_tag`，让等待态表现成独立状态标签而不是裸正文字符。一旦新的真实 delta 到达，`loading` 元素会立即清空。gateway 侧会分别缓存 `content` 与 `loading` 的上次已发送值，只在对应元素实际变化时才发 CardKit `content update`，减少 waiting tick 对正文的 no-op patch，也避免把单卡操作数白白耗在重复正文上。每次 CardKit 内容更新和关闭 settings 更新都带递增 `sequence`，创建 payload 不带 `header`。`phase=commentary` 的中途说明在 item completed 时只累积为当前 turn 流式前缀，不关闭卡、不进入 pending 普通文本重发；`phase=final_answer` 等 turn completed 后由最终 block 用完整正文关闭同一张流式卡，并先清空 `loading` 元素。工具调用、reasoning、执行过程和其它非白名单 assistant 文本不进入这条流式消息；普通 `im.v1.message.patch` 不再用于更新流式文本消息，避免命中普通消息不可 patch 成卡片的限制
