...
- turn-owned 的投递策略当前已经改成“高价值文本 reply、过程卡仍顶层 append”：
  ...
  - assistant-facing 的 `agent_message` delta 会在 upstream 标记 `phase=final_answer` 或 `phase=commentary` 时投影成 `assistant_stream` CardKit streaming card：同一 turn 的 item started 先创建 `streaming_mode=true` 的 CardKit 卡片实体并回复到 turn anchor，content 初始为空字符串，等待首段正文期间不再 patch 空格占位；后续 commentary / final delta 会先在 orchestrator 内维护“目标全文”和“当前可见前缀”两份状态，再只把可见前缀写入同一个 content 元素。这样即使飞书客户端没有触发原生逐字打印，服务端也会在本地 `100ms` heartbeat 上持续推进前缀，形成单调增长的打字机效果；同时仍保持每次更新都满足“旧文本是新文本前缀”这一 CardKit streaming 约束。卡片配置继续对齐飞书官方默认节奏：`print_frequency_ms=70`、`print_step=1`、`print_strategy=fast`；服务端显式把流式更新节流在单卡 `10 次/秒` 预算内，不再重发“完全相同的全文” no-op patch。每次 CardKit 内容更新和关闭 settings 更新都带递增 `sequence`，创建 payload 不带 `header`。`phase=commentary` 的中途说明在 item completed 时只累积为当前 turn 流式前缀，不关闭卡、不进入 pending 普通文本重发；`phase=final_answer` 等 turn completed 后由最终 block 用完整正文关闭同一张流式卡。工具调用、reasoning、执行过程和其它非白名单 assistant 文本不进入这条流式消息；普通 `im.v1.message.patch` 不再用于更新流式文本消息，避免命中普通消息不可 patch 成卡片的限制
