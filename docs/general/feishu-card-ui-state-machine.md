...
| review final card `进入审阅` / `放弃审阅` / `按审阅意见继续修改` | `mixed` | 按钮 payload 仍由 Feishu UI 生成 `page_action + daemon_lifecycle_id`，并继续复用同步 first-result replacement、freshness 与 old-card reject；真正的 detached review session 启动、review runtime 清理，以及把审阅结果带回 parent thread 继续修改，仍由 orchestrator 承接。当前只有携带 `file_change` 摘要的普通 final card 才会追加 `进入审阅`；review session 内的 final card 只保留 `放弃审阅` / `按审阅意见继续修改` 两个显式出口 |
...
3. `first_result_card` 路径
  ...
  - review final card 的 `进入审阅`、`放弃审阅`、`按审阅意见继续修改` 当前都落在这条路径上：按钮本身继续走 `page_action + daemon_lifecycle_id`，同步 replace 则完全复用既有 first-result 规则，不额外引入 review 专用 callback 通道
...
- turn-owned 的投递策略当前已经改成“高价值文本 reply、过程卡仍顶层 append”：
  ...
  - assistant-facing 的 `agent_message` delta 会在 upstream 标记 `phase=final_answer` 或 `phase=commentary` 时投影成 `assistant_stream` CardKit streaming card：同一 turn 的 item started 先创建 `streaming_mode=true` 的 CardKit 卡片实体并回复到 turn anchor，content 初始为空字符串，等待首段正文期间不再 patch 空格占位；卡体额外保留一个独立 `loading` 元素，优先展示运行时上传缓存的小尺寸三点 GIF。GIF 图片按飞书图片组件文档使用历史尺寸模式 `mode=tiny`，内置素材本身保持 `48x48` 正方形高密度三点动画，并沿用最初三点 GIF 的 `17/16/17` 帧间隔，避免 `24x8` 横向 GIF 在 tiny 图标位被客户端裁成单点，也避免移动端高 DPI 屏幕把低像素源图放大成模糊图；同时避免 `fit_horizontal` 横向撑满，也避免 `scale_type=crop_center` + `size=tiny/small` 裁剪成单点；显式像素 `size` 不再使用，因为它曾被 CardKit 拒绝（`img size is not allowed`）。后续 commentary / final delta 只把真实全量正文写入同一个 `content` 元素，不再把等待态拼回正文，确保每次更新都满足飞书“旧文本是新文本前缀”这一原生打字机渲染条件；等待态开关通过单独的 `loading` 元素全量更新完成，GIF 已显示时不再为动画 tick 反复 patch 该元素；daemon graceful shutdown 会在最终离线 notice 前先对仍打开的 `assistant_stream` 发送 `Done` close，清空 loading 元素并关闭 CardKit streaming mode，避免服务重启前最后一张卡片永久停留在 GIF loading 状态；上传得到的 `image_key` 会先缓存在 gateway 进程内，并按 GIF 内容 hash 落盘到当前 gateway 的图片暂存目录，daemon 重启后 hash 匹配即可复用，GIF 内容变更时自动重新上传。卡片配置 `print_frequency_ms=50`、`print_step=1`、`print_strategy=fast`，每次 CardKit 内容更新、loading 元素更新、刷新 settings 更新和关闭 settings 更新都带递增 `sequence`，创建 payload 不带 `header`，让飞书客户端按无标题正文卡逐字打印。若流式卡仍在等待后续 item 或后续 final，orchestrator 会按官方 `10 分钟` 自动关闭窗口预留 `30 秒` safety skew，在单张流式卡打开约 `9分30秒` 时主动发送 `refresh_stream_card`，gateway 对同一 `card_id` 调 settings 接口重新设置 `streaming_mode=true`，并继续保留 active stream；后续 final answer 仍写回这张原卡，不再新建第二张流式卡。若飞书在长流式过程中返回 `300309 streaming mode is closed` 或 `200850 card streaming timeout`，gateway 会先用 settings 接口递增 `sequence` 重新开启 `streaming_mode=true`，并对当前内容更新、刷新、关闭前内容写回或 loading 元素更新重试一次；重试仍命中终止态或 reopen 无法完成时，才清理本地 sequence/loading 状态并停止继续 patch，不把这类过期卡更新升级成 `gateway_apply_failure` 或 degraded 链路。`phase=commentary` 的中途说明在 item completed 时只累积为当前 turn 流式前缀，不关闭卡、不进入 pending 普通文本重发；`phase=final_answer` 等 turn completed 后由最终 block 先清空 loading 元素，再关闭同一张流式卡。工具调用、reasoning、执行过程和其它非白名单 assistant 文本不进入这条流式消息；普通 `im.v1.message.patch` 不再用于更新流式文本消息，避免命中普通消息不可 patch 成卡片的限制
...
| `old_card` | callback 带 `daemon_lifecycle_id` 且与当前 daemon 不匹配 | 发“旧卡片已过期” notice，不进入产品处理，也不会 replace 当前卡；review final card 的 `进入审阅` / `放弃审阅` / `按审阅意见继续修改` 也完全复用这条拒绝路径 |
