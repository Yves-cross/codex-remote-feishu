...
| `page_action` | `action_kind`、`action_arg(可选)`、`catalog_family_id(可选)`、`catalog_variant_id(可选)`、`catalog_backend(可选)` | page 卡按钮的结构化动作；gateway 解析后直接写回 `Action.Kind`，并用 `BuildFeishuActionText` 生成 canonical `Action.Text`；若 payload 携带 catalog provenance，也会同步写回 `Action.Catalog*`。当前 `/bendtomywill` success card 的“回滚最近一次修补”会用这条 payload 携带 `action_kind=surface.command.turn_patch.rollback` 与 `action_arg=patch_id` |
