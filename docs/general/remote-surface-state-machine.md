...
10. [internal/core/orchestrator/service_recovery.go](../../internal/core/orchestrator/service_recovery.go)
11. [internal/codexstate/sqlite_threads.go](../../internal/codexstate/sqlite_threads.go)
12. [internal/adapter/feishu/gateway_routing.go](../../internal/adapter/feishu/gateway_routing.go)
13. [internal/adapter/feishu/gateway.go](../../internal/adapter/feishu/gateway.go)
14. [internal/adapter/feishu/gateway_runtime.go](../../internal/adapter/feishu/gateway_runtime.go)
15. [internal/adapter/feishu/projector.go](../../internal/adapter/feishu/projector.go)
16. [internal/core/orchestrator/service_command_menu.go](../../internal/core/orchestrator/service_command_menu.go)
17. [internal/app/daemon/app_headless.go](../../internal/app/daemon/app_headless.go)
18. [internal/app/daemon/app_headless_restore_hints.go](../../internal/app/daemon/app_headless_restore_hints.go)
19. [internal/app/daemon/app_ingress.go](../../internal/app/daemon/app_ingress.go)
20. [internal/app/daemon/app_surface_resume_state.go](../../internal/app/daemon/app_surface_resume_state.go)
21. [internal/app/daemon/surface_resume_state.go](../../internal/app/daemon/surface_resume_state.go)
22. [internal/app/daemon/app_test.go](../../internal/app/daemon/app_test.go)
23. [internal/app/daemon/surface_resume_state_test.go](../../internal/app/daemon/surface_resume_state_test.go)
24. [internal/app/daemon/admin_vscode.go](../../internal/app/daemon/admin_vscode.go)
...
2. `ProductMode` 当前已经进入 daemon 级 `surface resume state`：
   ...
   5. `vscode` mode surface 会按 persisted `ResumeInstanceID` 继续尝试恢复：
      ...
      5. `vscode` mode 不会参与 headless auto-restore；`surface resume state` 在 load/write 时也会把所有非 `normal` mode entry 的 `ResumeHeadless` 强制归零，因此 `vscode` surface 不会保留可继续触发 headless 恢复的持久化目标。
...
6. 当前工作会话命令已经按 mode 分流：
   1. normal mode 主展示命令是 `workspace` 命令族：`/workspace` / `/workspace new` 负责父页导航，`/workspace list` / `/workspace new dir` / `/workspace new git` / `/workspace new worktree` 打开四张独立业务卡；旧 `/list` / `/use` / `/useall` 只是 alias。
...
5. 当前 review session 没有独立的 attach/list 暴露语义，也不会自动把 review thread 变成 surface 默认选中 thread；普通 attach/list/use 候选现在会显式过滤 `source=review` 的 detached review thread。

补充说明：

1. 当前还存在一个**可叠加**的 steering overlay：
   1. 某个 queued item 被点赞升级后，会离开 `QueuedQueueItemIDs`
   2. 或者用户 reply 当前 processing 的 source message，且 reply 内容属于当前 v1 支持的文本/图片输入时，会创建一个临时 steering item；独立文件 reply 当前不会走 steering，而是保留为 staged file
...
当前 normal mode 的工作会话主展示已经切到 `workspace` 命令族：bare `/workspace` / `/workspace new` 负责父页导航，`/workspace list` / `/workspace new dir` / `/workspace new git` / `/workspace new worktree` 负责四张独立业务卡；`/list` / `/use` / `/useall` 只保留 alias，并在 normal mode 下汇合到 `/workspace list`。
...
8. 旧的 normal-mode grouped workspace/thread selection cards 不再是主路径。
   1. 当前不再保留旧 `create_workspace` transport 兼容入口；normal mode 的新工作区路径统一走 `/workspace new dir` / `/workspace new git` / `/workspace new worktree` 这三张业务卡。
...
7. `PendingHeadless.AutoRestore=true` 时，手动 `/upgrade latest` 与 dev flavor 下的 `/upgrade dev` 检查结果 prompt 不再因为这条后台恢复占位被判成“当前窗口不空闲”；自动升级提示仍保持保守，不会优先挑这种 surface 弹卡。
...
8. transport degraded 后“attachment 仍保留但实例已离线”的 retained-offline 状态。
...
5. `vscode` surface 不参与 headless auto-restore；而且 `surface resume state` 会把非 `normal` mode 的 `ResumeHeadless` 硬归零，避免 daemon 重启后从持久化状态重新长出 headless 恢复入口。
...
2. 二级分组顺序稳定，但组内可见命令会按当前 `product mode + menu stage` 做 display projection：
   ...
   7. `/new` 与 `/bendtomywill` 只在 `normal` working 可见；`/status` 当前在 `基本命令`，`/history` 在 `常用工具`，`/bendtomywill` 在 `系统管理`，其中 `/status` 与 `/history` 在 normal / vscode 都可见。
...
5. `常用工具` 分组里的 `/cron`，以及 `系统管理` 分组里的 `/debug`、`/upgrade`、`/bendtomywill` 当前仍然是直接触发 daemon 动作的命令入口，不属于参数卡表单。
...
### 4.18 `/bendtomywill` 是 normal-mode 当前 thread 的前台事务卡，不回改已展示旧消息
...
R1 AttachedUnbound
  ...
  -- 文件消息(normal mode，workspace 已知) --> 隐式进入 R5，并先停留在 D1 StagedAttachments（不会仅凭文件创建新 thread）
...
6. target picker 当前承担的是 normal mode 下四张独立工作会话业务卡，而不是旧的 unified 大卡：
   ...
   7. `/workspace list` 当前主路径只会发出 `target_picker_select_workspace` / `target_picker_select_session`；`target_picker_select_mode` / `target_picker_select_source` 仍保留 transport 兼容，但主路径默认不会命中。`/workspace new dir` / `git` 会继续使用 `target_picker_open_path_picker` 打开目录子步骤；`/workspace new worktree` 则继续使用 `target_picker_select_workspace` / `target_picker_page` 切换与翻页基准工作区，并在同一张 owner card 内 inline replace 往返。
...
| `/workspace list` `/workspace new dir` `/workspace new git` `/workspace new worktree` | `normal`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`vscode`: 拒绝并提示先 `/mode normal` | `normal`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`vscode`: 拒绝并提示先 `/mode normal` | `normal`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`vscode`: 拒绝并提示先 `/mode normal` | `normal`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`vscode`: 拒绝并提示先 `/mode normal` | `normal`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`vscode`: 拒绝并提示先 `/mode normal` | `normal`: 允许，分别打开切换卡 / 目录新建卡 / Git 新建卡 / Worktree 新建卡；`vscode`: 拒绝并提示先 `/mode normal` |
...
| 文件 | 拒绝 | `normal`: 允许并隐式进入 `R5` 后暂存；`vscode`: 拒绝 | 允许 | 拒绝 | 允许 | 仅在首条文本尚未入队前允许 |
...
| 覆盖状态 | 当前行为 |
| --- | --- |
| `G1 PendingHeadlessStarting` | 只允许 `/status`、`/autowhip`、`/autocontinue`、`/debug`、`/upgrade`、`/mode`、`/detach`、revoke/reaction；其中 `/mode vscode` 会直接 kill 当前恢复流程并清空 headless restore 语义；reaction 即使放行到 action 层，也只会在满足 steering 条件时生效。若当前 pending 只是后台 auto-restore 占位，手动 `/upgrade latest` 与 dev flavor 下的 `/upgrade dev` 允许继续弹候选升级卡 |
...
| `show_threads` | `ActionShowThreads` | normal mode 下重新打开 `/workspace list` 切换卡；vscode mode 下仍是当前实例最近会话视图；两条路径当前都会默认排除 `source=review` 的 detached review thread |
| `show_all_threads` | `ActionShowAllThreads` | normal mode 下重新打开 `/workspace list` 切换卡；vscode mode 下仍是当前实例全部会话视图；两条路径当前都会默认排除 `source=review` 的 detached review thread |
