...
- 如果一次内容更新、关闭前内容写回或 loading 元素更新收到 `200850` 或 `300309`，gateway 可用递增 `sequence` 先调用 settings 接口重新开启 `streaming_mode=true`，再重试当前操作一次；重试仍命中终止态时必须清理本地 stream 状态并停止继续 patch，避免无限重试。
