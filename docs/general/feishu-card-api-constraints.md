...
- 因此长 turn 不能只依赖“一次打开后一直等到最终答复”。当前默认策略是：单卡打开接近官方 `10 分钟` 自动关闭窗口前，orchestrator 发送 `refresh_stream_card`，gateway 对同一 `card_id` 用递增 `sequence` 调 settings 接口续开 `streaming_mode=true`，并继续保留同一张卡作为后续正文渲染区域。
- 如果一次内容更新、刷新、关闭前内容写回或 loading 元素更新收到 `200850` 或 `300309`，gateway 可用递增 `sequence` 先调用 settings 接口重新开启 `streaming_mode=true`，再重试当前操作一次；重试仍命中终止态时必须清理本地 stream 状态并停止继续 patch，避免无限重试。
