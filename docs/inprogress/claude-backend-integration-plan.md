...
> Type: `inprogress`
> Updated: `2026-04-27`
> Summary: 合并此前三份 Claude 设计文档后，继续按当前仓库代码与最新 feidex 复核，收敛 Claude backend 基座缺口、命令策略边界与关键交互护栏，并同步 `#463` 已落地的 hello/backend/capability gate。
...
6. 状态层已有实例级 backend/capability snapshot carrier，但还没有形成完整 backend-aware state partition。
   - `internal/core/state/types.go`
7. surface resume 持久层还没有 backend 维度。
   - `internal/app/daemon/app_surface_resume_state.go`
   - `internal/app/daemon/surfaceresume/state.go`
8. daemon 启动时挂接的持久 thread catalog 仍然是 Codex-specific。
   - `internal/app/daemon/entry.go`
9. `/mode` 仍只有 `normal | vscode`。
...
2. state partition 仍未扩展到 workspace defaults / surface resume / thread-session 持久键。
3. command strategy matrix（native / approximation / passthrough / reject）仍未收口。
