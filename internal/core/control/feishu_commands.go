...
var feishuCommandSpecs = []feishuCommandSpec{
	...
	},
	{
		definition: FeishuCommandDefinition{
			...
			ArgumentKind:     FeishuCommandArgumentNone,
			Description:      "展示最近会话；normal attached 时只看当前工作区并附带“显示全部”按钮，normal detached 时等同 `/useall`，vscode detached 时需先 `/list`。",
	...
	},
	{
		definition: FeishuCommandDefinition{
			...
			ArgumentKind:     FeishuCommandArgumentNone,
			Description:      "展示全部会话；normal 模式下跨 workspace 列出，vscode attached 时仍只看当前实例，detached 时需先 `/list`。",
			ShowInHelp:       true,
			ShowInMenu:       true,
		},
		textExact: []feishuCommandMatch{
