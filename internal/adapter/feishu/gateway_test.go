	...
	gateway.fetchMessageFn = func(_ context.Context, messageID string) (*gatewayMessage, error) {
		...
		return &gatewayMessage{
			...
			Children: []*gatewayMessage{
				{MessageID: "om-forward-child-1", MessageType: "text", Content: `{"text":"/compact"}`},
				{MessageID: "om-forward-child-2", MessageType: "text", Content: `{"text":"已请求压缩当前线程上下文。"}`},
				{MessageID: "om-forward-child-3", MessageType: "text", Content: `{"text":"当前线程上下文已压缩完成。"}`},
			},
		}, nil
	}
	event := &larkim.P2MessageReceiveV1{
